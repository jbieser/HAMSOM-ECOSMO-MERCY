      ...
      subroutine strom(zinc,vtmit,vtmic,avmax,szahl,full_hour)
...
!        print*, "SINKING SHIP ",r_dt_dd(ll), ll
         SinkD_o=0.
         if( full_hour == 0 ) then
             vdpom = -0.00005787 !add calculation for average sinkingvelocity of POM and living phytoplankton

             SinkD_o( HGDET3D )   = -0.00005787 * r_dt_dd(ll)*3.!/2. !    5 m/day
             SinkD_o( HGPOC3D )   = -0.00005787 * r_dt_dd(ll)*3.! * 0.0!/2.
             SinkD_o( PTOM3D  )   = -0.00005787 * r_dt_dd(ll)*3.!/2.
             SinkD_o( HGS_P3D )   = -0.00005787 * 0.2 * r_dt_dd(ll)*3.!/2.
             SinkD_o( HGCYA3D )   =  0.000001157* r_dt_dd(ll)*3.!/2. ! -0.1 m/day upwelling
             SinkD_o( MEHGPOC3D ) = -0.00005787 * r_dt_dd(ll)*3.!/2.
             SinkD_o( MEHGDET3D ) = -0.00005787 * r_dt_dd(ll)*3.!/2.
             SinkD_o( MEHGCYA3D ) =  0.000001157* r_dt_dd(ll)*3.!/2.
        endif
        SinkD_u = SinkD_o             
!WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW
...
cc        Tc(ll,ibio)= XYadv(ll,ibio)+svert(ll,ibio)
        Tc(ll,ibio) =  
     &     sn(ll,ibio)
     &   + XYadv(ll,ibio)
     &   + svert(ll,ibio)

!      IF( Tc(ll,ibio) .GT. 10E20 .OR. Tc(ll,ibio) .LT. -10E20 ) THEN
!        print*, ll, ibio, VNAMOUT( ibio ), sn(ll,ibio),XYadv(ll,ibio),svert(ll,ibio),Tc(ll,ibio)
!      END IF
                end if
             end do
         end if
